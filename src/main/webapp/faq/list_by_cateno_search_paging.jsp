...
<script type="text/javascript">
 ...
 //content 클래스를 가진 div를 표시/숨김(토글)
   ...
   {
     $(".toggle_none").not($(this).next(".toggle_none").slideToggle(100)).slideUp();
   });
 });
</script> 
</head>
              ...
              <div class="search_faq_section">
                ...
                <c:choose>
                  <c:when test="${param.word != '' }"> 
                    <input type="text" class="iptxt" value='${param.word }' placeholder="search.." >
                  </c:when>
                  <c:otherwise>
                    <input type="text" class="iptxt" value='' placeholder="search.." >
                  </c:otherwise>
                </c:choose>
                  <button type="button" class="btn_search">찾기</button>
                  <c:if test='${param.word.length()>0 }'>
                    <button type='button' onclick="location.href='./list_all.do?faqno=${faqVO.faqno}&word='">검색 취소</button>  
                  </c:if>
                </fieldset>
              </div>
