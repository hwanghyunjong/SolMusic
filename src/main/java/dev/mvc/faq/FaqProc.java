...
public class FaqProc implements FaqProcInter{
  ...
  public String pagingBox(String listFile, int faqno, int search_count, int nowPage, String faq_word){ 
    int totalPage = (int)(Math.ceil((double)search_count/Faq.RECORD_PER_PAGE)); // 전체 페이지  
    int totalGrp = (int)(Math.ceil((double)totalPage/Faq.PAGE_PER_BLOCK));// 전체 그룹 
    int nowGrp = (int)(Math.ceil((double)nowPage/Faq.PAGE_PER_BLOCK));    // 현재 그룹 
    int startPage = ((nowGrp - 1) * Faq.PAGE_PER_BLOCK) + 1; // 특정 그룹의 페이지 목록 시작  
    int endPage = (nowGrp * Faq.PAGE_PER_BLOCK);             // 특정 그룹의 페이지 목록 종료   
...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
    ...
    if (nowGrp >= 2){ 
      str.append("<span class='span_box_1'><A href='"+listFile+"?faq_word="+faq_word+"&nowPage="+_nowPage+"&faqno="+faqno+"'>이전</A></span>"); 
    } 
    ...
    if (nowGrp < totalGrp){ 
      str.append("<span class='span_box_1'><A href='"+listFile+"?faq_word="+faq_word+"&nowPage="+_nowPage+"&faqno="+faqno+"'>다음</A></span>"); 
    } 
    str.append("</DIV>"); 
